...
function MapView({ homeCountry, idleMode, onIdleModeChange }: {
  ...
  const { revealedSet, arrivedSet, arrivedCount, total, phase, waitingForGesture } =
    useRevealSequence(countryId ?? null, mode)
  ...
  return (
    ...
    >
      ...
      {/* Cold load from a shared link or a reload: no browser will start audio
          before the visitor interacts, so instead of playing the reveal to a
          muted room we hold it and let their first click BE the start. This
          catcher swallows that click so it can't land on a country and change
          the selection someone shared. Any keypress works too — the unlock
          listener in sounds.ts fires on keydown as well. */}
      {waitingForGesture && (
        <div
          onClick={e => { e.stopPropagation(); ensureAudioReady() }}
          className="absolute inset-0 z-50 flex items-end justify-center pb-8 cursor-pointer"
        >
          <span className="flex items-center gap-2.5 px-5 h-11 rounded-full select-none
            text-[11px] tracking-[0.22em] uppercase
            bg-white/65 dark:bg-white/[0.08] backdrop-blur-xl backdrop-saturate-150
            border border-white/60 dark:border-white/10
            shadow-lg shadow-black/[0.07] dark:shadow-black/40
            text-[#6a6054] dark:text-[#a8a29e] animate-prompt-breathe">
